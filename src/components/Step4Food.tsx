      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--text-muted)] text-sm sm:text-base max-w-xl mx-auto">
            Plan highway restaurants and food stops between <strong className="text-[var(--text-primary)]">{fromLocation}</strong> and{' '}
            <strong className="text-[var(--text-primary)]">{toLocation}</strong>.
          </p>
        </div>
        ...
        <div className="ui-card p-6 sm:p-8 mb-8 text-center max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="text-[11px] text-[var(--text-muted)]">Highway diners & dhabas</div>
            </button>
            ...
            >
              ...
              <div className="text-[11px] text-[var(--text-muted)]">Non-stop journey (₹0)</div>
            </button>
          </div>
        </div>
        ...
        {wantsFood === true && (
          <div className="space-y-6 animate-fade-in mb-8">
            ...
            <div className="ui-card p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-sky-500/10 text-sky-500 flex items-center justify-center">
                  <ShoppingBag className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-xs text-[var(--text-muted)]">Meal Calculation Target:</div>
                  <div className="text-sm sm:text-base font-bold text-[var(--text-primary)]">
                    Calculating for <span className="text-sky-600 dark:text-sky-400">{peopleLabel}</span>
                  </div>
                </div>
              </div>
            ...
            {selectedFoodItems.length > 0 && (
              <div className="ui-card p-5 border-sky-500/30">
                <div className="flex items-center justify-between pb-3 border-b border-[var(--border-color)] mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[var(--text-primary)]">Selected Food Items</span>
                    <span className="px-2 py-0.5 rounded-full bg-sky-500/10 text-sky-600 dark:text-sky-400 text-[10px] font-bold">
                      {selectedFoodItems.length} Item(s)
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] uppercase font-semibold text-[var(--text-muted)] block">Food Subtotal</span>
                ...
                <div className="space-y-2 divide-y divide-[var(--border-color)]">
                      ...
                      <div key={item.id} className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {item.restaurantName && (
                              <span className="text-[var(--text-muted)] text-[11px]">at {item.restaurantName}</span>
                            )}
                          </div>
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-3">
                          <div className="flex items-center gap-1.5 bg-[var(--bg-surface-elevated)] border border-[var(--border-color)] px-2 py-0.5 rounded-lg">
                            <button
                              type="button"
                            ...
                            <span className="font-bold text-[var(--text-primary)] px-1 text-xs">{item.quantity || 1}</span>
                            <button
                              type="button"
                          ...
                          <span className="font-bold text-sky-600 dark:text-sky-400 text-sm min-w-[70px] text-right">
                            {formatINR(itemTotal)}
                          </span>
                          ...
                          <button
                            ...
                            className="p-1 rounded hover:bg-red-500/10 text-[var(--text-muted)] hover:text-red-500 cursor-pointer"
                          >
              ...
              {foodStops.map((stop) => {
                ...
                return (
                  <div
                    ...
                    className={`ui-card group relative overflow-hidden transition-all flex flex-col justify-between ${
                      isSelectedStop ? 'ui-card-selected' : 'ui-card-hover'
                    }`}
                  >
                    <div className="relative h-40 w-full overflow-hidden bg-[var(--bg-surface-elevated)]">
                      <img
                        src={stop.image}
                        alt={stop.name}
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <div className="text-xs text-[var(--text-muted)] mb-3">
                          <span>{stop.location}</span> • <span className="text-sky-500 font-medium">{stop.cuisine}</span>
                        </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-[var(--border-color)]">
                          <div className="text-xs font-bold text-[var(--text-primary)] mb-2 flex items-center justify-between">
                            ...
                            <span className="text-[10px] text-[var(--text-muted)] font-normal">Per Person Rate</span>
                          </div>
                          ...
                          <div className="space-y-1.5">
                            {stopMenuItems.map((menuItem) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full ${menuItem.isVeg ? 'bg-emerald-500' : 'bg-red-500'}`} />
                                    <div>
                                      <div className="text-[var(--text-primary)] font-medium">{menuItem.name}</div>
                                  ...
                                  <div>
                                    {alreadySelected ? (
                                      <span className="text-xs text-emerald-600 dark:text-emerald-400 font-bold px-2 py-1 bg-emerald-500/10 rounded-lg">
                                      ...
                                      <button
                                        ...
                                        className="ui-btn-secondary py-1 px-2.5 text-xs"
                                      >
                                        <Plus className="w-3 h-3" />
                                        <span>Add</span>
                                      </button>
                      ...
                      <div className="pt-3 mt-4 border-t border-[var(--border-color)] flex items-center justify-between">
